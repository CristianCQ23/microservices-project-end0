...
module.exports = (app) => {
    ...
    app.use(
        '/customer/wishlist',
        forward('customers', CUSTOMERS_URL, (req) =>
            req.originalUrl.replace(/^\/customer(?=\/|$)/, '/customers')
        )
    );
    app.use(
        '/customer',
    ...
    app.use(
        '/wishlist',
        forward('customers', CUSTOMERS_URL, (req) =>
            req.originalUrl.replace(/^\/wishlist(?=\/|$)/, '/customers/wishlist')
        )
    );
    app.use(
        '/cart',
        forward('customers', CUSTOMERS_URL, (req) =>
            req.originalUrl.replace(/^\/cart(?=\/|$)/, '/customers/cart')
        )
    );
